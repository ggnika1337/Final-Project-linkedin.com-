...
function NewPostPopup({
  ...
  return (
    <>
      ...
      >
        <div className="flex gap-3 mx-[24px] cursor-pointer whitespace-nowrap p-[15px] max-h-[80px] flex rounded-[10px] hover:bg-[#80808034]">
        ...
        <div className="pl-[24px] min-h-[150px] w-full h-full relative">
          <textarea
            value={text}
            onChange={(e) => setText(e.target.value)}
            className="focus:outline-none w-full h-full transition-none min-h-[200px] max-h-[400px]"
          />
          <div
            className={`overflow-hidden max-w-[320px] ${vip ? "h-[290px]" : "h-[0px]"} shadow-lg`}
          >
            <PremiumPopup onClick={() => setVip(false)} />
          </div>
          <div
        ...
        <div className="px-[24px] flex active:outline-none">
          <button
            onClick={() => {
              setText("Text that was rewritten by AI :O");
              setVip(true);
            }}
            className="rounded-[15px] flex items-center justify-center gap-1 text-[13px] border-1 border-black px-2 cursor-pointer hover:bg-[#8080802f]"
          >
            <svg
              width={16}
              height={16}
          ...
          />
          <PostAdditions label="More" svg="M19 11h-6V5h-2v6H5v2h6v6h2v-6h6z" />
        </div>
        <div className="w-full h-[1px] bg-[#80808030]"></div>
        <div className="px-[24px] flex items-end justify-end">
          <button
            className={`rounded-[15px] px-[10px] text-[14px] py-0.5 ${text.length > 0 ? "bg-[#0b67c2] text-white" : "bg-[#e8e8e8] text-[#aeafae]"}`}
          >
            Post
          </button>
        </div>
